...
async function getContributorInfo(project) {
    ...
    try {
        ...
        if (!contributorsResponse.ok) {
            console.warn("Could not fetch site contributors.");
        ...
        if (contributors) {
            contributors.forEach(c => {
                ...
                if (c.role === 'OWNER') ownerContactId = c.contactId;
            });
        }
    } catch (error) {
...
export async function onRequestPost({ request, env }) {
    ...
    try {
        ...
        // --- ACTION: LIST MEMBERS (with owner protection) ---
        if (action === 'list') {
        ...
        // --- ACTION: DELETE MEMBERS (safe, two-step process) ---
        if (action === 'delete') {
            const logs = [];
            const memberChunks = [];
